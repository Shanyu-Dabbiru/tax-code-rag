"""Embedding utilities for tax code content."""

from __future__ import annotations

from typing import List, Dict, Any

from opentelemetry import trace
from sentence_transformers import SentenceTransformer
from fastembed import SparseTextEmbedding


class TaxEmbedder:
	"""Embed tax code text using a sentence-transformers model and fastembed."""

	def __init__(self) -> None:
		self.dense_model_name = "BAAI/bge-small-en-v1.5"
		self.sparse_model_name = "Qdrant/bm25"
		self.device = "cpu"
		self._dense_model = SentenceTransformer(self.dense_model_name, device=self.device)
		self._sparse_model = SparseTextEmbedding(self.sparse_model_name)
		self._tracer = trace.get_tracer(__name__)

	def get_embedding_dim(self) -> int:
		"""Get the embedding dimension (vector size) of the dense model."""
		return self._dense_model.get_sentence_embedding_dimension()

	def embed_text(self, text: str) -> List[float]:
		"""Embed a single text string into a dense vector."""
		with self._tracer.start_as_current_span("embed_text") as span:
			span.set_attribute("text_length", len(text))
			span.set_attribute("model_name", self.dense_model_name)
			span.set_attribute("device", self.device)
			vector = self._dense_model.encode(
				text,
				convert_to_numpy=True,
				show_progress_bar=False,
			)
			return vector.tolist()

	def embed_batch(self, texts: List[str]) -> List[List[float]]:
		"""Embed a batch of text strings into dense vectors."""
		with self._tracer.start_as_current_span("embed_batch") as span:
			span.set_attribute("batch_size", len(texts))
			span.set_attribute("model_name", self.dense_model_name)
			span.set_attribute("device", self.device)
			vectors = self._dense_model.encode(
				texts,
				batch_size=32,
				convert_to_numpy=True,
				show_progress_bar=False,
			)
			return vectors.tolist()

	def embed_sparse_batch(self, texts: List[str]) -> List[Dict[str, Any]]:
		"""Embed a batch of text strings into sparse vectors using BM25."""
		with self._tracer.start_as_current_span("embed_sparse_batch") as span:
			span.set_attribute("batch_size", len(texts))
			span.set_attribute("model_name", self.sparse_model_name)
			# fastembed returns an iterator of SparseEmbedding objects
			sparse_embeddings_iter = self._sparse_model.embed(texts)
			
			result = []
			for emb in sparse_embeddings_iter:
				result.append({
					"indices": emb.indices.tolist(),
					"values": emb.values.tolist(),
				})
			return result

